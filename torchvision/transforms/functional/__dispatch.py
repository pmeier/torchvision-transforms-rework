...
def _horizontal_flip_bounding_box(input: features.BoundingBox) -> features.BoundingBox:
    return features.BoundingBox(
        F.horizontal_flip_bounding_box(input, image_size=input.image_size), like=input, format="xyxy"
...
def _resize_image(input: features.Image, *, size: Tuple[int, int], interpolation_mode: str) -> features.Image:
    if interpolation_mode is FEATURE_SPECIFIC_DEFAULT:
...
def _resize_bounding_box(input: features.BoundingBox, *, size: Tuple[int, int], **_: Any) -> features.BoundingBox:
    return features.BoundingBox(
        F.resize_bounding_box(input, old_image_size=input.image_size, new_image_size=size),
        like=input,
        image_size=size,
        format="xyxy",
    ).to_format(input.format)
...
def _resize_segmentation_mask(
    input: features.SegmentationMask, *, size: Tuple[int, int], interpolation_mode: str
